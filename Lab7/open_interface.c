...
void oi_moveForward(oi_t *sensor_data, float cm){
    int count = 0;
    //int i;
    char bumpedLeft[] = "Bumped Left";
    char bumpedRight[] = "Bumped Right";
    float mm = cm*10;
    while(count<mm){
        oi_update(sensor_data);
        int i = 0;
        if(sensor_data->bumpLeft){
            collisionBumpLeft(sensor_data);
            for( i=0; i <strlen(bumpedLeft); i++){
                uart_sendChar(bumpedLeft[i]);
            ...
            collisionBumpRight(sensor_data);
            for( i=0; i <strlen(bumpedRight); i++){
                uart_sendChar(bumpedRight[i]);
        }
            uart_sendChar('\r');
            uart_sendChar('\n');
            break;
...
void oi_moveBackward(oi_t *sensor_data, float cm){
    float sum= 0;
    float mm = cm*10;
    while(sum>mm){
        oi_update(sensor_data);
    ...
    void oi_turnCounterClockwise(oi_t *sensor_data, float deg){
        volatile float sum_angle = 0;
        while(sum_angle<deg){
            oi_update(sensor_data);
            sum_angle += sensor_data->angle;
            //lcd_printf("%d", sum_angle);

        }
    }


    void oi_turnClockwise(oi_t *sensor_data, float deg){
        volatile float sum_angle = 0;
            while(sum_angle>deg){
                oi_update(sensor_data);
                sum_angle += sensor_data->angle;
