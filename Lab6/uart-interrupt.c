...
void uart_interrupt_init(void){
  ...
  GPIO_PORTB_PCTL_R = 0x0000011;

  //calculate baud rate
  uint16_t iBRD = 8; //use equations
  uint16_t fBRD = 0.68055 * 64 + 0.5; //use equations
...
void uart_sendChar_interrupt(char data){
...
void uart_sendStr_interrupt(const char *data){
...
void UART1_Handler(void){
    ...
    if (UART1_MIS_R & 0x0010)
    {
        ...
        UART1_ICR_R |= 0b00010000; // change


        //read the byte received from UART1_DR_R and echo it back to PuTTY
        //ignore the error bits in UART1_DR_R        UART1_DR_R  |= 0x07
       // data = (char)(UART1_DR_R & 0xFF);
